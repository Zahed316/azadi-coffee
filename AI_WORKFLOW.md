...
## Project Purpose
Azadi Coffee is a Persian-first bilingual ecommerce storefront for a minimalist specialty coffee roastery. It is a Next.js App Router app with FA routes at `/`, EN routes under `/en`, static seed data, live CSS-variable theming, localStorage cart state, and a Zarinpal payment flow.
...
- `src/lib` — Locale helpers, theme settings, cart/order logic, formatting, SEO schema, Zarinpal client.
- `src/data` — Typed seed data for products and posts. No database or CMS is used.
...
- Keep product and blog content in `src/data` unless a task explicitly changes the data strategy.
...
- Do not introduce WordPress, WooCommerce, a database, or external CMS assumptions.
